...
describe SizesController do
  ...
  def mock_size(stubs={})
    (@mock_size ||= mock_model(Size).as_null_object).tap do |size|
      size.stub(stubs) unless stubs.empty?
    end
  end

  describe "GET index" do
    it "assigns all sizes as @sizes" do
      Size.stub(:all) { [mock_size] }
      get :index
      assigns(:sizes).should eq([mock_size])
    end
  end

  describe "GET show" do
    it "assigns the requested size as @size" do
      Size.stub(:find).with("37") { mock_size }
      get :show, :id => "37"
      assigns(:size).should be(mock_size)
    end
  end

  describe "GET new" do
    it "assigns a new size as @size" do
      Size.stub(:new) { mock_size }
      get :new
      assigns(:size).should be(mock_size)
    end
  end
  ...
  describe "POST create" do
    ...
    describe "with valid params" do
      it "assigns a newly created size as @size" do
        Size.stub(:new).with({'these' => 'params'}) { mock_size(:save => true) }
        post :create, :size => {'these' => 'params'}
        assigns(:size).should be(mock_size)
      end

      it "redirects to the created size" do
        Size.stub(:new) { mock_size(:save => true) }
        post :create, :size => {}
    ...
    describe "with invalid params" do
      it "assigns a newly created but unsaved size as @size" do
        Size.stub(:new).with({'these' => 'params'}) { mock_size(:save => false) }
        post :create, :size => {'these' => 'params'}
        assigns(:size).should be(mock_size)
      end

      it "re-renders the 'new' template" do
        Size.stub(:new) { mock_size(:save => false) }
        post :create, :size => {}
  ...
  describe "PUT update" do
    ...
    describe "with invalid params" do
      it "assigns the size as @size" do
        Size.stub(:find) { mock_size(:update_attributes => false) }
        put :update, :id => "1"
        assigns(:size).should be(mock_size)
      end
